                        ...
                        <c:forEach items="${info.list}" var="p">
                            <tr>
                                ...
                                <td>${p.pNumber}</td>
                                    ...
                                    <%--&nbsp;&nbsp;&nbsp;<a href="${pageContext.request.contextPath}/admin/product?flag=update&pid=${p.pId}">修改</a></td>--%>
                                <td>
                                    <button type="button" class="btn btn-info "
                                            onclick="update(${p.pId},${info.pageNum})">编辑
                                    </button>
...
<script type="text/javascript">
    ...
    function deleteBatch(pageNum) {
        ...
        if(cks.length === 0){
            ...
            //拼接ID
                ...
                //发送ajax请求到服务器端
                $.ajax({
                    url: "${pageContext.request.contextPath}/productInfo/deletebatch.action",
                    data: {
                        "str": str,
                        "page":pageNum,
                        "pName":pName,
                    ...
                    success: function (msg) {
                        alert(msg);
                        $("#table").load("http://127.0.0.1:8080/mimiShoppingMall/admin/product.jsp #table");
                    }
                })
            }
        }
    ...
    function del(pid, pageNum) {
        if (confirm("确定删除吗")) {
          ...
          //向服务器提交请求完成删除
            $.ajax({
                url: "${pageContext.request.contextPath}/productInfo/delete.action",
                data: {
                    "pid": pid,
                    "page":pageNum,
                    "pName":pName,
    ...
    function update(pid, page) {
        ...
        let param = "?pid=" + pid + "&page=" + page + "&pName=" + pName + "&typeId=" + typeId + "&lPrice=" + lPrice + "&hPrice=" + hPrice;

        location.href = "${pageContext.request.contextPath}/productInfo/update.action" + param;
    }
</script>
<!--分页的AJAX实现-->
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        //异步ajax分页请求
        $.ajax({
        url:"${pageContext.request.contextPath}/productInfo/ajaxsplit.action",
            data:{
                "page":page,
                "pName":pName,
                "typeId":typeId,
